...
router.get("/profile/me", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const userResult = await query<any>(
      "SELECT id, username, email, profile_picture, bio, is_public, profile_category, created_at, updated_at FROM users WHERE id = $1",
      [userId]
    );
    ...
    const user: User = {
      ...
      profileCategory: dbUser.profile_category ?? undefined,
      createdAt: new Date(dbUser.created_at),
      updatedAt: new Date(dbUser.updated_at),
    };

    // Current user can always view their own collection
...
router.get("/:username", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Récupère l'utilisateur
    const userResult = await query<any>(
      "SELECT id, username, email, profile_picture, bio, is_public, profile_category, created_at, updated_at FROM users WHERE username = $1",
      [username]
    );
...
router.put("/profile/update", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const { username, bio, profilePicture, isPublic, profileCategory } = req.body;

    // Check if username is being changed
    if (username) {
      const currentUserResult = await query<any>(
        ...
        if (currentUser.last_username_change) {
          ...
          if (diffDays < 30) {
            return res.status(400).json({
              error: `Vous ne pouvez modifier votre pseudo qu'une fois tous les 30 jours. Prochain changement possible dans ${30 - diffDays} jours.`
            });
          }
        }

        // Update last_username_change
        await query(
          "UPDATE users SET last_username_change = NOW() WHERE id = $1",
          [req.user?.userId]
        );
      }
    }
    ...
    await query(
      `UPDATE users SET username = $1, bio = $2, profile_picture = $3, is_public = $4, profile_category = $5, updated_at = NOW()
        WHERE id = $6`,
      [username, bio, profilePicture, isPublic, profileCategory, req.user?.userId]
    );
